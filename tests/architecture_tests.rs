...
use render_sandbox::render_graph::{RenderGraph, PlaceholderPass, ResourceUsage};
use render_sandbox::scene::{Scene, SceneNode, Transform, NodeContent, Camera};
...
fn test_render_graph_compilation() {
    ...
    let pass1 = PlaceholderPass::new("Pass1")
        .with_resource("ResourceA", ResourceUsage::Write);
    let pass2 = PlaceholderPass::new("Pass2")
        .with_resource("ResourceA", ResourceUsage::Read);
...
fn test_scene_camera() {
    ...
    // Create camera node
    let camera = Camera::perspective(45.0, 16.0 / 9.0, 0.1, 100.0);
    let camera_node = SceneNode::with_name("MainCamera")
        .with_content(NodeContent::Camera(camera));
