...
fn test_gltf_error_types() {
    ...
    assert_eq!(format!("{}", unsupported_error), "Unsupported feature: test feature");
}
...
fn test_gltf_error_display() {
    ...
    assert_eq!(format!("{}", err), "Unsupported feature: orthographic cameras");
}
...
fn test_triangle_gltf_file_exists() {
    ...
    if triangle_path.exists() {
        let _content = std::fs::read_to_string(triangle_path).expect("Should be able to read test file");
